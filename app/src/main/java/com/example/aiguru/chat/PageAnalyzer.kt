...
/**
 * Analyzes an educational image/page using Groq's vision model and returns
 * a structured [PageContent] with:
 *
 *  • Full verbatim transcript of all text on the page
 *  • Per-paragraph: number, full text, one-line summary
 *  • Per-diagram/figure: heading, concept context, visible elements, depiction,
 *    position on page, all labelled parts
 *  • Key terms / concepts extracted from the page
 *
 * Always uses [GroqApiClient] (vision model: llama-4-scout) regardless of the
 * active chat client, because the server proxy doesn't accept image bytes.
 *
