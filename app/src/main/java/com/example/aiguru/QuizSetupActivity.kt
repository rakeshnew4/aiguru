    ...
    val obj = org.json.JSONObject().apply {
        ...
        questions.forEach { q ->
            val qObj = org.json.JSONObject().apply {
                ...
                when (q) {
                    is com.example.aiguru.models.QuizQuestion.MCQ -> {
                        val opts = org.json.JSONArray()
                        q.options.forEach { opts.put(it) }
                        put("options", opts)
                        put("correct_answer", q.correctAnswer)
                    }
                    is com.example.aiguru.models.QuizQuestion.FillBlankTyped -> {
                    ...
                    is com.example.aiguru.models.QuizQuestion.ShortAnswer -> {
